...
class CreatePet:
    ...
    def create_pet(self, slack_user_id, pet_name):
        ...
        pet = Pet(
            user_id=user.user_id, pet_name=pet_name, hp=self.default_pet_starting_hp
        )
        db.session.add(pet)
        db.session.commit()
    ...
    def feed_pet(self, slack_user_id, inventory_id):
        ...
        pet = db.session.query(Pet).filter_by(user_id=user.user_id).first()
        inventory = (
            db.session.query(Inventory).filter_by(inventory_id=inventory_id).first()
        )
        product = (
            db.session.query(Product).filter_by(product_id=inventory.product_id).first()
        )
        ...
        db.session.query(Inventory).filter_by(inventory_id=inventory_id).update(
            {"quantity": inventory.quantity - 1}
        )
        db.session.commit()

        # update pet hp
        db.session.query(Pet).filter_by(id=pet.id).update(
            {"hp": pet.hp + product.price}
        )
        db.session.commit()
