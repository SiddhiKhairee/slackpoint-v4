...
class TaskDone:
    ...
    def update_points(self):
        ...
        # if task is not done
            ...
            current_user = (
                db.session.query(User).filter_by(slack_user_id=current_slack_id).one()
            )
            ...
            # Get current task
            curr_task = (
                db.session.query(Task).filter_by(task_id=current_task_id).first()
            )

            # Add task points to points_earned in "user" table
            db.session.query(User).filter_by(user_id=user_id).update(
                dict(
                    points_earned=getattr(curr_task, "points")
                    + getattr(current_user, "points_earned")
                )
            )
            ...
            if player_existent:
                ...
                curr_player = (
                    ...
                    .filter_by(player_id=getattr(my_query, "player_id"))
                    .first()
                )
                # Update the values for the player
                db.session.query(Player).filter_by(
                    player_id=getattr(curr_player, "player_id")
                ).update(
                    dict(
                        stat_points_to_allocate=getattr(curr_task, "points")
                        + getattr(curr_player, "stat_points_to_allocate")
                    )
                )
